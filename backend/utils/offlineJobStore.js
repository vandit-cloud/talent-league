...
// Seed data - written on first run
const SEED_JOBS = [
    {
        _id: 'job_1',
...
if (!fs.existsSync(JOBS_FILE)) {
    fs.writeFileSync(JOBS_FILE, JSON.stringify(SEED_JOBS, null, 2));
}
...
const getJobs = () => {
    try {
        ...
        console.error('Error reading offline jobs:', err);
        return [];
    }
};
...
const saveJobs = (jobs) => {
    try {
        ...
        console.error('Error saving offline jobs:', err);
    }
};
...
module.exports = { getJobs, findJobById, addJob, deleteJob };
